...
export const INTERVIEW_TOPIC_BACKLOG: InterviewTopic[] = [
  ...
  },
  {
    ...
    guidance:
      "Whenever a city gets flagged as loved or avoided (via city-sentiment, from the picker or the conversation), don't stop at the sentiment — follow up on why, once, referencing the specific city by name: weather, being near family or friends, hotel quality there, how short the layover usually runs, or just nothing to do on downtime. A pilot who avoids a city because the layover is always too short needs a completely different fix from one who avoids it because the hotel is bad, and the ranking tool can't tell the difference without asking. Capture the why as a qualitative fact tied to that city, using the same city-sentiment binding's code — don't invent a new mechanism for it.",
  },
  {
  ...
  },
  {
    ...
    guidance:
      "Go beyond the single circadianHealth lean: ask about consecutive early or late report days specifically (does two or three early mornings in a row wear on them differently than one isolated early report), which is exactly what distinctReportHoursPerTrip and backOfClockDeparturesPerTrip are for — bind to those implicit ids from their answer, don't invent new ones. Back-to-back circadian-disruptive trips (one rough trip immediately followed by another, across the calendar) cannot be measured — trip-to-trip calendar adjacency isn't reliably tracked by this app's parser — so if a pilot raises that specifically, capture it as qualitative only, don't imply it's being scored.",
  },
  {
