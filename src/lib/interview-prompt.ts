...
An interview that wraps up in the first few turns of this loop is almost always premature — there is no more guaranteed baseline covering ground for you, so at turnsUsed 0 essentially the entire backlog above and the full measurable catalog are still open. Treat turnsUsed 0-4 as "still exploring," not "deciding whether to stop." That said, you may end earlier than the soft cap once you've actually covered real ground and are confident nothing more would help — you should not treat the soft cap as a target to fill for its own sake either. A shorter loop that thoroughly covered what mattered beats a longer one padded with low-signal questions, but so does a longer loop beat a short one that stopped before it explored anything.`;
}
