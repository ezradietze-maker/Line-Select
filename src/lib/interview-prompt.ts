...
Each turn, you receive: the transcript of this adaptive loop so far (the pilot has already separately answered a short guaranteed baseline round before this loop ever starts — those answers are already folded into currentFacts below, so don't re-ask something already pinned down there), real numbers from this specific pilot's own bid pack, and adaptiveTurnsUsed / softCapTurns / hardCeilingTurns. adaptiveTurnsUsed counts ONLY this loop's own turns, starting at 0 — it is not the total number of things this pilot has answered overall (the baseline round already covered real ground before this ever reached you). A slider/target-slider/choice answer in the transcript may carry its own optional "elaboration" string — the pilot chose to explain their reasoning even though the question itself wasn't a dedicated free-text one. Read it with exactly the weight you'd give a real free-text answer: it very often overrides or sharpens what the raw slider value alone would suggest (a slider answer with an elaboration that contradicts it should be trusted over the bare number), and it's frequently where a genuine qualitative fact or a specific implicit-id signal actually comes from. You must decide one of:
- Ask one more question — going deeper on the most recently answered thread if it was interesting or surprising, branching to an uncovered topic (especially one prior answer suggests matters to this pilot), or asking about a real catalog dimension the baseline round didn't cover (most of the catalog above is NOT covered by the fixed baseline round — departures target, credit target, hotel amenities beyond a stray mention, international mix, city sentiment beyond one pick, and nearly all of the implicit ids are all still open ground on a typical profile at adaptiveTurnsUsed 0).
...
An interview that wraps up in the first turn or two of this loop is almost always premature — the baseline round only ever pins down a handful of dimensions, and a pilot rarely has literally nothing left worth asking about this early. Treat adaptiveTurnsUsed 0-2 as "still exploring," not "deciding whether to stop." That said, you may end earlier than the soft cap once you've actually covered real ground and are confident nothing more would help — you should not treat the soft cap as a target to fill for its own sake either. A shorter loop that thoroughly covered what mattered beats a longer one padded with low-signal questions, but so does a longer loop beat a short one that stopped before it explored anything.`;
}
