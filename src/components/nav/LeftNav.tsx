    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 px-3">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-md border-l-[3px] py-2.5 pl-3 pr-3 text-left text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                isActive
                  ? "border-accent bg-accent-wash font-semibold text-accent"
                  : "border-transparent font-medium text-ink-muted hover:bg-black/[0.035] hover:text-ink"
              }`}
            >
        ...
        <button
          ...
          className="mt-3 flex w-full items-center gap-3 rounded-md border-l-[3px] border-transparent py-2.5 pl-3 pr-3 text-left text-sm font-medium text-ink-faint transition-colors hover:bg-black/[0.035] hover:text-ink-muted"
        >
    ...
    <>
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-sidebar-border bg-sidebar px-4 py-3 md:hidden">
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-md text-ink-muted hover:bg-black/[0.05]"
        >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          />
          <div className="absolute inset-y-0 left-0 w-72 max-w-[85vw] animate-fade-in border-r border-sidebar-border bg-sidebar shadow-elevated-lg">
            {sidebarContent}
          </div>
        </div>
      )}
